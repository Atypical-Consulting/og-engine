...
Generates an image by fetching metadata from a URL — the zero-config approach. OG Engine fetches Open Graph tags (`og:title`, `og:description`, `og:image`, etc.) from the target URL and uses them to populate the card automatically.

```typescript
og.renderFromUrl(request: RenderFromUrlRequest): Promise<Buffer>
```

### RenderFromUrlRequest type

```typescript
interface RenderFromUrlRequest {
  url: string
  format?: 'og' | 'twitter' | 'square' | 'linkedin' | 'story'
  template?: TemplateName
  style?: RenderRequest['style']
  output?: RenderRequest['output']
  overrides?: Partial<Pick<RenderRequest, 'title' | 'description' | 'author' | 'tag'>>
}
```
...
```typescript
// Zero-config — title and description pulled from the page's OG tags
const imageBuffer = await og.renderFromUrl({
  url: 'https://myblog.com/posts/my-latest-article',
  format: 'og',
  style: { gradient: 'deep-sea', font: 'Outfit' },
})

await Bun.write('article-card.png', imageBuffer)

// With overrides — override just the tag while keeping scraped title/description
const imageBuffer2 = await og.renderFromUrl({
  url: 'https://myblog.com/posts/my-latest-article',
  format: 'twitter',

...
```json
{
  "items": [
    ...
    },
    {
      ...
      "title": "{{name}}",
      "variables": { "name": "Jane Smith", "handle": "@janesmith" },
      "images": { "avatar": "https://example.com/avatars/jane.jpg" }
    }
  ]
}
```
...
Each item in `items` is a full render request body. See [POST /render](/api-reference/render/) for the complete field reference. Items can have different formats, templates, and styles. Each item may also include `variables` and `images` fields to supply template interpolation data or named server-fetched images for that specific render.

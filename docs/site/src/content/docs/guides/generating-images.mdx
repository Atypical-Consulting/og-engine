...
Total time from request to response: typically **2–5ms** for a standard OG image.
...
```json
{
  ...
  "description": "Pure JS text measurement replaces Puppeteer. Sub-5ms renders, zero browser dependencies.",
...
curl -I -X POST https://api.og-engine.com/render \
  -H "Authorization: Bearer oge_sk_YOUR_KEY" \
  -H "Content-Type: application/json" \
...
- Read [Formats & Templates](/guides/formats-and-templates/) to understand the 5 format presets and 4 templates

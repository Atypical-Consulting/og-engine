...
description: Headless Chrome costs ~130ms (warm) and 200MB per OG image. We built OG Engine with Pretext to render the same images in ~22ms with 10MB — up to 30x faster. Here's why and how.
head:
  - tag: script
    attrs:
    ...
    content: |
      {
        ...
        "description": "Headless Chrome costs ~130ms (warm) and 200MB per OG image. We built OG Engine with Pretext to render the same images in ~22ms with 10MB — up to 30x faster.",
        "author": {
          "@type": "Organization",
          "name": "OG Engine",
...
Total: **~22ms per image** ([benchmarked](/benchmarks/)). PNG encoding dominates; text layout + canvas draw take under 1ms. No browser, no DOM, no Xvfb. A single Node.js process handles 500+ concurrent renders.
